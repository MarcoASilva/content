...
function addgamepad(gamepad) {
  ...
  console.log(
    /^(.*)\s*\(.*vendor:?\s*([0-9a-f][0-9a-f]?[0-9a-f]?[0-9a-f]?)\s+product:?\s*([0-9a-f][0-9a-f]?[0-9a-f]?[0-9a-f]?)\s*\)\s*$/i.exec(
      gamepad.id
    )
  );
...
function scangamepads() {
  ...
  console.log(gamepads, document.querySelector("#noDevices").style.display);
